...
class Auth
{
   ...
   public function __construct(\PDO $dbh, $config, $language = "en_GB")
   {
      ...
      if (version_compare(phpversion(), '5.4.0', '<')) {
         die('PHP 5.4.0 required for PHPAuthentication engine!');
      }
      
      $this->password = new Pass( $config );
   ...
   public function login($email, $password, $remember = 0, $captcha = NULL)
   {
      ...
      $validatePassword = $this->password->validate($password);
   ...
   */
   public function changePassword($uid, $currpass, $newpass, $repeatnewpass, $captcha = NULL)
   {
      ...
      $validatePassword = $this->password->validate($currpass);
      ...
      $validatePassword = $this->password->validate($newpass);
